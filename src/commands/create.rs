...
use crate::utils::{print_success, verify_uv_installed};
use std::fs;
...
pub(crate) fn run(name: String, python_version: Option<&str>) -> Result<()> {
    ...
    // Clean up unnecessary files generated by uv init
    let _ = fs::remove_file(env_path.join("main.py"));
    let _ = fs::remove_file(env_path.join(".gitignore"));
    let _ = fs::remove_dir_all(env_path.join(".git"));
